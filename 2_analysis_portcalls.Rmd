...
# Data inladen
load("D:/Projecten/19_0003_Zicht_op_afvallozingen_met_AIS/Data/1_reading_cleaning/Portcalls_EU_1518_preproc.Rda")
```
...
Volgende stap is het vergelijken van schepen die naar dezelfde Nederlandse haven terugkeren

```{r rdam_return_vs_rest, echo = FALSE, message = FALSE, warning = FALSE}

# Selectie schepen die in Rotterdam zijn geweest; schepen die binnen 4 uur terug zijn gekeerd labelen
portcalls_selectie <-
  portcalls %>%
  filter(Port.Name == "Rotterdam") %>%
  mutate(return_4h = if_else(Time_travel_h < 4,1,0))
